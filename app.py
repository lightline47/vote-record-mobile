...
# BUILD: vote-record-mobile v5.88 PRESENCE-POSITION-FIX 2026-09-07
...
ADMIN_PASSWORD_STORE_KEY = "vote_record_admin_password"
APP_ACCESS_PASSWORD_STORE_KEY = "vote_record_app_access_password"
...
# ============================================================
# 실시간 접속 현황 (Supabase heartbeat)
# - 브라우저/탭별 session_id를 발급하고 마지막 활동시각을 저장합니다.
# - 10초마다 heartbeat를 갱신하고 최근 25초 이내 신호가 있는 세션만 '현재접속중'으로 계산합니다.
# - '오늘접속'은 한국시간(Asia/Seoul) 00:00 이후 접속한 고유 세션 수입니다.
# ============================================================
PRESENCE_PREFIX = "vote_presence_"
PRESENCE_ACTIVE_SECONDS = 25
PRESENCE_HEARTBEAT_SECONDS = 10
KST = ZoneInfo("Asia/Seoul")


def _presence_session_id():
    if "presence_session_id_v1" not in st.session_state:
        st.session_state.presence_session_id_v1 = uuid.uuid4().hex
    return st.session_state.presence_session_id_v1


def presence_heartbeat(selected_station_key=None):
    if not supabase_persistence_ready():
        return
    sid = _presence_session_id()
    now_utc = datetime.now(timezone.utc)
    first_key = "presence_first_seen_v1"
    if first_key not in st.session_state:
        st.session_state[first_key] = now_utc.isoformat()
    payload = {
        "session_id": sid,
        "station_key": str(selected_station_key or ""),
        "first_seen": st.session_state[first_key],
        "last_seen": now_utc.isoformat(),
    }
    try:
        save_persistent_json(PRESENCE_PREFIX + sid, payload)
    except Exception as exc:
        st.session_state["presence_warning_v1"] = str(exc)


def _presence_rows():
    if not supabase_persistence_ready():
        return []
    # app_key가 vote_presence_ 로 시작하는 행만 조회
    pattern = urllib.parse.quote(PRESENCE_PREFIX + "*", safe="")
    try:
        rows = _supabase_request("GET", f"?select=app_key,payload,updated_at&app_key=like.{pattern}")
...
def presence_counts(selected_station_key=None):
    now_utc = datetime.now(timezone.utc)
    active_cutoff = now_utc - timedelta(seconds=PRESENCE_ACTIVE_SECONDS)
    ...
    for row in _presence_rows():
        payload = row.get("payload") if isinstance(row, dict) else None
        if not isinstance(payload, dict):
            continue
        sid = str(payload.get("session_id") or row.get("app_key") or "")
        try:
            last_seen = datetime.fromisoformat(str(payload.get("last_seen") or row.get("updated_at") or "").replace("Z", "+00:00"))
            if last_seen.tzinfo is None:
                last_seen = last_seen.replace(tzinfo=timezone.utc)
        except Exception:
            continue
        try:
            first_seen = datetime.fromisoformat(str(payload.get("first_seen") or "").replace("Z", "+00:00"))
        ...
        if last_seen >= active_cutoff:
            active_all.add(sid)
            if station_key and str(payload.get("station_key") or "") == station_key:
                active_station.add(sid)
        # 오늘 00:00 이후 한 번이라도 활동한 세션. first_seen이 이전이어도 last_seen이 오늘이면 포함.
        if last_seen >= midnight_utc:
            today.add(sid)

    return len(active_all), len(active_station), len(today)


@st.fragment(run_every=f"{PRESENCE_HEARTBEAT_SECONDS}s")
def render_presence_heartbeat(selected_station_key=None, mode="station"):
    """앱이 열린 동안 10초마다 heartbeat를 보내고 현재 접속 현황만 갱신합니다."""
    presence_heartbeat(selected_station_key)
    active_all, active_station, today_count = presence_counts(selected_station_key)
    if mode == "station":
        st.markdown(
            f'<div class="station-presence-inline-v588">현재접속중:{active_station}명</div>',
            unsafe_allow_html=True,
        )
    elif mode == "admin":
        st.markdown(
            f'<div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap;margin:0;">'
            f'<span style="font-size:.95rem;font-weight:400;color:#333;">현재접속중:{active_all}명</span>'
            f'<span style="font-size:.95rem;font-weight:400;color:#333;">오늘접속:{today_count}명</span>'
            f'</div>',
            unsafe_allow_html=True,
        )


# v5.89: 투표소 현재접속 표시는 '선택한 투표소는~입니다.' 확인상자 바로 아래 왼쪽에 표시
st.markdown("""
<style>
.station-presence-inline-v588 {
...
def _save_persistent_password(store_key, local_file, session_key, password):
    """Supabase 저장 성공 후에만 캐시와 로컬 보조본을 변경합니다."""
    clean_password = str(password).strip()
    if not clean_password:
        raise ValueError("비밀번호를 입력해 주세요.")
    save_persistent_json(store_key, {"password": clean_password})
    st.session_state[session_key] = clean_password
    local_file.write_text(
        json.dumps({"password": clean_password}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )


def load_admin_password():
    return _load_persistent_password(
        ADMIN_PASSWORD_STORE_KEY,
        ADMIN_FILE,
        "cached_admin_password_v602",
    )


def save_admin_password(password):
    _save_persistent_password(
        ADMIN_PASSWORD_STORE_KEY,
        ADMIN_FILE,
        "cached_admin_password_v602",
        password,
...
def load_app_access_password():
    return _load_persistent_password(
        APP_ACCESS_PASSWORD_STORE_KEY,
        APP_ACCESS_FILE,
        "cached_app_access_password_v602",
    )
            ...
            if new_dong and new_station:
                ...
                ent = station_db.setdefault(
                    key,
                    {
                        "dong": new_dong,
                        "station": new_station,
                        # JSON/Supabase 저장 후에도 엑셀의 최초 투표구 행 순서를 보존합니다.
                        "excel_order": len(station_db),
                        "registered": reg,
                        "elections": [],
                    },
                )
...
def natural_text_key(value):
    """'갑9동' 다음에 '갑10동'이 오도록 문자와 숫자를 나누어 정렬합니다."""
    return tuple(
...
def excel_order_value(entry):
    """엑셀에서 처음 나타난 투표구 순번. 이전 자료에는 없을 수 있습니다."""
    try:
        return int(entry.get("excel_order"))
...
def format_serial_list_with_no(value):
    """복수 일련번호 각각을 'No. 1,234' 형식으로 표시합니다."""
    raw = str(value or "").strip()
    tokens = re.findall(r"\d{1,3}(?:,\d{3})+|\d+", raw)
    if not tokens:
        return ""
    return ", ".join(
        f"No. {int(token.replace(',', '')):,}" for token in tokens
    )
...
# 휴대전화에서 투표소 선택 후 콤보박스 위치에 화면이 남지 않도록
# 재실행 직후 페이지 최상단으로 복원하여 제목과 상단 메뉴가 모두 보이게 합니다.
if st.session_state.pop("restore_mobile_menu_after_station_select_v596", False):
    components.html(
        """
        <script>
        function restoreVoteAppTopV601() {
          try {
        ...
        restoreVoteAppTopV601();
        setTimeout(restoreVoteAppTopV601, 100);
        setTimeout(restoreVoteAppTopV601, 300);
        setTimeout(restoreVoteAppTopV601, 700);
        </script>
        """,
        height=0,
        width=0,
...
# 접속 현황은 아래 st.fragment가 10초마다 독립적으로 갱신합니다.
...
if station is not None:
    selected_station_confirm_html_v530 = (
        ...
        '<div class="selected-station-confirm shared-station-confirm" style="margin-bottom:0 !important;">'
...
if workflow_step not in ("①[선택]투표소", "②[보고]투표진행상황", "[관리자]"):
    with workflow_cluster_v59:
        ...
        render_presence_heartbeat(selected_key, mode="station")
...
if workflow_step == "①[선택]투표소":
    ...
    with workflow_cluster_v59:
        with st.container(key="selection_body_v531"):
            ...
            # 동 콤보박스는 엑셀에서 각 동의 투표구가 처음 등장한 순서로 표시합니다.
            # 이전 버전에서 등록해 excel_order가 없는 자료는 자연 숫자 순서로 정렬합니다.
            dong_entries = {}
            for v in db.values():
                dong_entries.setdefault(v["dong"], []).append(v)

            def dong_combo_sort_key(dong_name):
                orders = [
                    excel_order_value(v) for v in dong_entries[dong_name]
                    if excel_order_value(v) is not None
                ]
                if orders:
                    return (0, min(orders), natural_text_key(dong_name))
                return (1, 0, natural_text_key(dong_name))

            dongs = sorted(dong_entries.keys(), key=dong_combo_sort_key)
            ...
            if not dongs:
                ...
                with st.container(border=True, key="station_choice_input_box"):
                    ...
                    if selected_dong:
                        selected_dong_entries = [
                            v for v in db.values() if v["dong"] == selected_dong
                        ]
                        selected_dong_entries.sort(
                            key=lambda v: (
                                0,
                                excel_order_value(v),
                            ) if excel_order_value(v) is not None else (
                                1,
                                station_number(v["station"]),
                            )
                        )
                        stations = [v["station"] for v in selected_dong_entries]
...
if workflow_step == "②[보고]투표진행상황":
    ...
    with workflow_cluster_v59:
        if not db:
            ...
            for idx, e in enumerate(station["elections"]):
                ...
                damaged_serial_display = format_serial_list_with_no(damaged_serial)
                ...
                if not valid:
                    ...
                    if damaged_count >= 1 and damaged_serial_display:
                        # ③의 훼손 등 미교부 매수가 1 이상인 경우에만
                        # 입력한 일련번호를 ④의 같은 칸에 함께 표시합니다.
                        serial_text = (
                            f"{remain_range}"
                            f"<br><span style='font-size:13px;'>{damaged_serial_display}</span>"
                        )
                    else:
    ...
    with workflow_cluster_v59:
        st.markdown(
            '<div style="font-size:1.55rem;font-weight:900;color:#111;margin:0 0 2px 0;">[관리자]</div>',
            unsafe_allow_html=True,
        )
        render_presence_heartbeat(selected_key, mode="admin")
            ...
            if st.button("🔑 로그인 비밀번호 변경", width="stretch", key="change_access_pw_button_v585"):
                if current_access_pw != current_app_access_password():
                    ...
                    try:
                        ...
                        st.error(f"로그인 비밀번호를 영구 저장하지 못했습니다: {exc}")
                    else:
                        st.success("초기화면 로그인 비밀번호가 변경되어 영구 저장되었습니다. 다음 실행부터 새 비밀번호를 사용해 주세요.")
            ...
            if change_admin_pw_submitted:
                if current_pw_input != current_admin_password():
                    ...
                    try:
                        ...
                        st.error(f"관리자 비밀번호를 영구 저장하지 못했습니다: {exc}")
                    else:
                        st.success("관리자 비밀번호가 변경되어 영구 저장되었습니다. 앞으로 새 비밀번호를 사용해 주세요.")
...
# ============================================================
# 2026-09-15 모바일 상단 ①~④ 메뉴 오른쪽 글자 잘림 보정
# - 카드 내부 문구가 오른쪽 테두리에 닿아 잘리지 않도록 폭/여백 보정
# - 메뉴 기능 및 배치는 유지
# ============================================================
st.markdown(
    r"""
    ...
    @media (max-width: 768px) {
        ...
        /* 이전 버전의 선택 메뉴 밑줄 연장용 오른쪽 padding이 글자를 밀어내지 않도록 해제 */
        .st-key-navcard_select_v35 button[aria-pressed="true"] p,
        .st-key-navcard_report_v35 button[aria-pressed="true"] p,
        .st-key-navcard_input_v35 button[aria-pressed="true"] p,
        .st-key-navcard_reference_v35 button[aria-pressed="true"] p {
...
# ============================================================
# 2026-09-15 모바일 상단 메뉴 글자크기 복원 + 우측 잘림 방지 / 접속표시 간격 축소
# - 메뉴 본문 글자는 기존 크기(① 13px, ②~④ 12.2px) 유지
# - 자간/내부 여백/overflow만 보정하여 마지막 글자 잘림 방지
# - 현재접속중 표시는 확인상자 바로 아래, 기존 보이는 간격의 약 1/4 수준으로 축소
# ============================================================
st.markdown(
    r"""
    <style>
    @media (max-width: 768px) {
        /* 카드와 내부 요소에서 글자 끝을 잘라내지 않도록 함 */
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"],
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] > div,
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] button,
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] button > div {
            overflow: visible !important;
        }
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] button {
        ...
        /* 선택 카드 밑줄용 padding이 글자를 오른쪽으로 밀지 않도록 유지 */
        .st-key-navcard_select_v35 button[aria-pressed="true"] p,
        .st-key-navcard_report_v35 button[aria-pressed="true"] p,
        .st-key-navcard_input_v35 button[aria-pressed="true"] p,
        .st-key-navcard_reference_v35 button[aria-pressed="true"] p {
        ...
        /* Streamlit fragment/markdown이 만드는 상단 기본 공백을 줄여 확인상자에 밀착 */
        div[data-testid="stElementContainer"]:has(.station-presence-inline-v588),
        div[data-testid="stMarkdownContainer"]:has(.station-presence-inline-v588) {
...
# ============================================================
# 2026-09-15 추가 보정
# - ② 투표진행상황 마지막 '황' 잘림 방지: 본문을 왼쪽으로 소폭 이동
# - 현재접속중: 확인 네모상자 선과 겹치지 않고 바로 아래에 표시
# ============================================================
st.markdown(
    r"""
    <style>
    @media (max-width: 768px) {
        /* ② 카드 본문만 왼쪽으로 약간 이동 (글자 크기는 유지) */
        .st-key-workflow_cluster_v59 .st-key-navcard_report_v35 button p {
            position: relative !important;
            left: -2px !important;
            font-size: 12.2px !important;
        ...
        /* 접속자 표시는 확인상자 아래쪽으로 내려 선과 겹치지 않게 함 */
        .station-presence-inline-v588 {
            position: relative !important;
            margin-top: 0 !important;
            margin-left: 2px !important;
...
# 2026-09-15 최종 추가 보정
# - ② 투표진행상황 검정 메뉴명 복원
# - ①~④ 검정 메뉴명 크기를 기존 수준으로 확대
# - 카드 안에서 마지막 글자가 잘리지 않도록 폭/자간/overflow 보정
# - 현재접속중 표시를 확인상자 바로 아래에서 조금 더 위로 이동
# ============================================================
st.markdown(
    r"""
    <style>
    @media (max-width: 768px) {
        /* 카드 내부는 끝 글자가 잘리지 않도록 여유를 허용 */
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"],
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] > div,
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] button,
        .st-key-workflow_cluster_v59 [class*="st-key-navcard_"] button > div {
