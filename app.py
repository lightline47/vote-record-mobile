...
# 현재 세션의 접속상태를 갱신하고 전체/투표소별/오늘 접속자 수를 계산합니다.
presence_heartbeat(selected_key)
_active_all_count, _active_station_count, _today_count = presence_counts(selected_key)
...
if station is not None:
    selected_station_confirm_html_v530 = (
        ...
        '</div>'
        f'<span style="color:#333;font-size:.50em;font-weight:400;white-space:nowrap;">현재접속중:{_active_station_count}명</span>'
        '</div>'
    )
else:
    selected_station_confirm_html_v530 = (
...
if workflow_step == "②[보고]투표진행상황":
    ...
    with workflow_cluster_v59:
        st.markdown(
            f'<div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap;margin:0 0 6px 0;">'
            f'<span style="font-size:1.55rem;font-weight:900;color:#111;">[관리자]</span>'
            f'<span style="font-size:.95rem;font-weight:850;color:#333;">현재접속중:{_active_all_count}명</span>'
            f'<span style="font-size:.95rem;font-weight:850;color:#333;">오늘접속:{_today_count}명</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
