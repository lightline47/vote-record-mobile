...
# BUILD: vote-record-mobile FINAL v3.2 2026-08-24
...
# ============================================================
# v3.2 상단/메뉴 - native Streamlit 버튼 기반
# - 각 메뉴 카드 안의 동그라미 버튼은 실제 st.button이므로 메뉴 전환이 확실히 동작
# - 텍스트는 별도 HTML로 렌더링하여 ①~④ 및 [ ]를 보라색·굵게 표시
# - URL 이동을 사용하지 않아 로그인 session_state가 유지됨
# ============================================================
st.markdown(
    r"""
    <style>
    /* Streamlit 상단 툴바 아래로 본문을 충분히 내림 */
    .block-container {
        max-width: 1380px !important;
    ...
    .workflow-guide-v32 .first-step {color:#ff1717 !important; font-weight:950 !important;}
    .workflow-guide-v32 .next-steps {color:#102ed7 !important; font-weight:950 !important;}

    /* 메뉴 카드 컨테이너 */
    .st-key-nav_select_v32, .st-key-nav_report_v32,
    .st-key-nav_input_v32, .st-key-nav_reference_v32 {
        border: 2px solid #7b159d !important;
        border-radius: 9px !important;
    ...
    .st-key-nav_select_v32:has(.selected-nav-v32),
    .st-key-nav_report_v32:has(.selected-nav-v32),
    .st-key-nav_input_v32:has(.selected-nav-v32),
    .st-key-nav_reference_v32:has(.selected-nav-v32) {
    ...
    .nav-label-v32 .purple {color:#7b159d !important; font-weight:950 !important; font-size:18px !important;}
    .nav-label-v32 .black {color:#111 !important; font-weight:800 !important; font-size:16px !important;}
    .nav-label-v32.admin .black {color:#111 !important; font-weight:950 !important; font-size:18px !important;}
    .nav-arrow-v32 {
        min-height:68px;
        display:flex;
        align-items:center;
    ...
    /* 실제 메뉴 버튼은 동그라미로 보이게 */
    .st-key-nav_select_v32 button,
    .st-key-nav_report_v32 button,
    .st-key-nav_input_v32 button,
    .st-key-nav_reference_v32 button,
    .st-key-nav_admin_v32 button {
    ...
    .st-key-nav_select_v32:has(.selected-nav-v32) button,
    .st-key-nav_report_v32:has(.selected-nav-v32) button,
    .st-key-nav_input_v32:has(.selected-nav-v32) button,
    .st-key-nav_reference_v32:has(.selected-nav-v32) button,
    .st-key-nav_admin_v32:has(.selected-nav-v32) button {
        color: #ff4b4b !important;
        border-color: #ff4b4b !important;
    }

    /* 투표소 선택 옆의 예전 별표 문구는 완전히 숨김 */
    .section-row .must-select, .section-row .must-red, .section-row .must-blue {display:none !important;}
    ...
    @media (max-width: 900px) {
        ...
        .app-main-title-v32 {font-size:27px !important; line-height:1.42 !important;}
        .workflow-guide-v32 {font-size:17px !important;}
        .nav-label-v32 .purple {font-size:14px !important;}
        .nav-label-v32 .black {font-size:12px !important;}
        .nav-arrow-v32 {font-size:25px !important; min-height:62px !important;}
    }
    </style>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    '<div class="app-main-title-v32">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v3.2</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="workflow-guide-v32">★ 반드시 '
...
if "workflow_step_v32" not in st.session_state:
    st.session_state.workflow_step_v32 = "select"
if st.session_state.workflow_step_v32 not in WORKFLOW_LABELS:
    st.session_state.workflow_step_v32 = "select"


def _go_step_v32(step):
    st.session_state.workflow_step_v32 = step


def _menu_item_v32(step, lead, tail, admin=False):
    selected = st.session_state.workflow_step_v32 == step
    # key를 가진 container에 selected class용 marker를 넣고, 실제 버튼을 사용
    with st.container(key=f"nav_{step}_v32"):
        if selected:
            st.markdown('<span class="selected-nav-v32" style="display:none"></span>', unsafe_allow_html=True)
        bcol, tcol = st.columns([0.22, 1.78], gap="small")
        with bcol:
            st.button(
                "●" if selected else "○",
                key=f"navbtn_{step}_v32",
                on_click=_go_step_v32,
                args=(step,),
                help=f"{lead}{tail}",
            )
        with tcol:
            cssclass = "nav-label-v32 admin" if admin else "nav-label-v32"
            st.markdown(
                f'<div class="{cssclass}"><span class="purple">{lead}</span>'
                f'<span class="black">{tail}</span></div>',
                unsafe_allow_html=True,
            )


# ① → ② → ③ → ④
c1, a1, c2, a2, c3, a3, c4 = st.columns([1.15, .16, 1.55, .16, 1.55, .16, 1.55], gap="small")
with c1:
    _menu_item_v32("select", "①[선택]", " 투표소")
with a1:
    st.markdown('<div class="nav-arrow-v32">➜</div>', unsafe_allow_html=True)
with c2:
    _menu_item_v32("report", "②[보고자료]", " 투표진행상황")
with a2:
    st.markdown('<div class="nav-arrow-v32">➜</div>', unsafe_allow_html=True)
with c3:
    _menu_item_v32("input", "③[입력]", " 투표록 기초자료")
with a3:
    st.markdown('<div class="nav-arrow-v32">➜</div>', unsafe_allow_html=True)
with c4:
    _menu_item_v32("reference", "④[참고]", " 투표록 2p 작성")

# 관리자: 검정색 굵게
admin_col, _ = st.columns([1.15, 5.0])
with admin_col:
    _menu_item_v32("admin", "", "[관리자]", admin=True)

workflow_step = WORKFLOW_LABELS[st.session_state.workflow_step_v32]
