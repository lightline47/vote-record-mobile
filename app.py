...
# BUILD: vote-record-mobile v5.56 PAGE1-ARROW-SPACING-FIX 2026-08-25
            ...
            if not db or station is None:
                ...
                with st.container(border=True, key="report_input_calc_box"):
                    ...
                    for i, e in enumerate(report_elections):
                        ...
                        with in2:
                            st.markdown(
                                '<div class="report-v41-label-red report-input-label-v557 report-damaged-label-v573">[입력]<br>'
                                '훼손 등 미교부 투표용지 매수</div>',
                                unsafe_allow_html=True,
                            )
                ...
                for e in station["elections"]:
                    ref_rows.append(
                        "<tr>"
                        f"<td>{display_election_name_for_report(e['name'])}</td>"
                        f"<td>{e['received']:,}</td>"
                        f"<td>{e['start_no']:,}</td>"
                        f"<td>{e['end_no']:,}</td>"
                        "</tr>"
