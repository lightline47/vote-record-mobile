...
# BUILD: vote-record-mobile v5.86 MOBILE-TABLE-AUTOFOCUS-ADMIN 2026-08-26
...
if workflow_step == "②[보고]투표진행상황":
    ...
    with workflow_cluster_v59:
        if not db:
            ...
            # 각 선거의 입력이 완료된 경우 잔여매수를 선거끼리 직접 비교합니다.
            # 서로 다른 모든 조합을 표 바로 아래 빨간 안내문으로 표시합니다.
            remaining_rows = []
            for idx, e in enumerate(station["elections"]):
            ...
            for left_idx in range(len(remaining_rows)):
                for right_idx in range(left_idx + 1, len(remaining_rows)):
                    ...
                    if left_remaining != right_remaining:
                        remaining_mismatch_messages.append(
                            f"{left_name}와 {right_name}의 잔여매수가 불일치합니다."
                        )

            if remaining_mismatch_messages:
                st.markdown(
                    '<div style="color:#d40000;font-size:17px;font-weight:900;'
                    'line-height:1.5;margin:8px 0 12px 0;">'
                    + '<br>'.join(f"⚠ {message}" for message in remaining_mismatch_messages)
                    + '</div>',
                    unsafe_allow_html=True,
                )
    ...
    with workflow_cluster_v59:
        ...
        if not st.session_state.admin_authenticated:
            ...
            if st.button("🔑 비밀번호 변경", width="stretch", key="change_admin_pw_button_v585"):
