...
# BUILD: vote-record-mobile FINAL v4.5 INPUT-LAYOUT 2026-08-24
...
st.markdown(
    '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v4.5</div>',
    unsafe_allow_html=True,
)
st.markdown(
...
st.markdown(
    ...
    .st-key-navcard_report_v35 button::after {content:"②[보고자료]";}
    .st-key-navcard_input_v35 button::after {content:"③[입력]";}
    .st-key-navcard_reference_v35 button::after {content:"④[참고]";}
...
st.markdown(
    ...
    .station-choice-label .hint {color:#e11; font-size:18px; font-weight:800;}
    ...
    .election-choice-label span {color:red; font-size:20px; font-weight:900;}
...
c1, a1, c2, a2, c3, a3, c4, nav_spacer = st.columns([0.92, .11, 1.28, .11, 1.28, .11, 1.28, .80], gap="small")
with c1:
...
with a3:
    st.markdown('<div class="nav-arrow-v35">➜</div>', unsafe_allow_html=True)
with c4:
    st.button("투표록 2p 작성", key="navcard_reference_v35", on_click=_go_step_v35, args=("reference",), width="stretch")

admin_col, _ = st.columns([1.15, 5.0])
with admin_col:
    st.button("[관리자]", key="navcard_admin_v35", on_click=_go_step_v35, args=("admin",), width="stretch")
...
# v4.5: 선택된 투표소는 ②~④ 및 관리자 메뉴에서도 같은 위치에 항상 표시
...
if workflow_step == "①[선택]투표소":
    ...
    if not dongs:
        ...
        with st.container(border=True, key="station_choice_input_box"):
            ...
            with c1:
                st.markdown(
                    '<div class="station-choice-label"><span class="star">★</span>동선택 '
                    '<span class="hint">(아래 콤보박스에서 해당 동을 선택)</span></div>',
                    unsafe_allow_html=True,
                )
            ...
            with c2:
                st.markdown(
                    '<div class="station-choice-label"><span class="star">★</span>투표소 선택 '
                    '<span class="hint">(아래 콤보박스에서 해당 투표소를 선택)</span></div>',
                    unsafe_allow_html=True,
                )
    ...
    st.markdown(
        """
        <style>
        .report-v41-headrow{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:2px 0 10px 0;}
        .report-v41-title{display:inline-block;border:2px solid #222;border-radius:5px;padding:7px 12px;font-size:24px;font-weight:900;line-height:1.2;background:#fff;}
        .report-v41-target{display:inline-block;background:#edf5ff;color:#1768b3;padding:3px 6px;font-size:14px;font-weight:800;}
        .report-v41-station{font-size:22px;font-weight:900;color:#7b159d;text-decoration:underline;text-underline-offset:4px;margin:6px 0 12px 2px;}
        .report-v41-box-title{text-align:center;font-size:21px;font-weight:900;margin:0 0 10px 0;}
        .report-v41-label-red{color:red;font-size:18px;font-weight:900;line-height:1.22;margin-bottom:5px;}
        .report-v41-label-blue{color:blue;font-size:18px;font-weight:900;line-height:1.22;margin-bottom:5px;text-align:center;}
        ...
        .report-v41-notice{font-size:18px;font-weight:900;margin:12px 0 26px 0;line-height:1.4;}
        .report-v41-notice .blue{color:blue;font-weight:900;}
        .report-v41-ref-title{display:inline-block;border:2px solid #222;border-radius:5px;padding:7px 11px;font-size:20px;font-weight:900;line-height:1.25;margin:0 0 10px 0;background:#fff;}
        .report-v41-ref-title .station{color:#7b159d;text-decoration:underline;text-underline-offset:3px;}
        .st-key-report_input_calc_box{border:2px solid #1737ff !important;border-radius:6px !important;padding:10px 12px 9px 12px !important;margin:0 0 10px 0 !important;}
        ...
        @media(max-width:768px){
          ...
          .report-v41-label-red,.report-v41-label-blue,.report-v41-label-black{font-size:15px;}
    ...
    if not db or station is None:
        st.markdown('<div class="report-v41-headrow"><span class="report-v41-title">1. 투표진행상황 보고</span></div>', unsafe_allow_html=True)
        ...
        st.markdown(
            f'<div class="report-v41-headrow">'
            f'<span class="report-v41-title">1. 투표진행상황 보고</span>'
            f'<span class="report-v41-target">보고대상 선거: <b>{report_target_name}</b></span>'
            f'</div>'
            f'<div class="report-v41-station">{station_display}</div>',
            unsafe_allow_html=True,
        )
        ...
        with st.container(border=True, key="report_input_calc_box"):
            st.markdown('<div class="report-v41-box-title">[ 보고용 투표용지 교부수량 입력 / 산출 ]</div>', unsafe_allow_html=True)

            for i, e in enumerate(report_elections):
                c1, c2, c3 = st.columns([1.35, 0.95, 0.95], gap="small")

                with c1:
                    st.markdown('<div class="report-v41-label-red">[입력]<br>잔여투표용지 첫 번호(NO.)</div>', unsafe_allow_html=True)
                    raw = st.text_input(
                        "현재 잔여투표용지 첫 번호(NO.)",
                        value="",
                ...
                with c2:
                    st.markdown('<div class="report-v41-label-blue">[보고대상]<br>교부수량</div>', unsafe_allow_html=True)
                    if invalid_input:
                ...
                st.markdown(
                    '<div class="report-v41-help">➜ 보고 대상 선거의 현재 '
                    '<span class="red">남아 있는 투표용지 첫 번호(NO.)만</span> 입력합니다.</div>',
                    unsafe_allow_html=True,
                )
    ...
    if not db:
        ...
        st.markdown(
            ...
            '<span class="small">(투표록2p)</span> &apos;아. 투표상황&apos; 기초자료 입력</div>',
            unsafe_allow_html=True,
        )
        st.caption(
            "거소투표용지 미발송 반송자(2), 결정서 지참자(3), 거소투표용지와 회송용봉투 반납자(4)를 입력하면 "
            "투표록 '아.투표상황'의 선거인명부 등재자(1)와 계(나)가 자동 생성 됩니다."
        )

        st.markdown(
            '<div class="election-choice-label">★ 선거명 선택 '
            '<span>(아래 콤보박스에서 입력할 선거명 선택)</span></div>',
            unsafe_allow_html=True,
        )
        a_choice = st.selectbox(
            "선거명",
            ["선거명을 선택하세요"] + election_names,
            index=0,
            key=f"a_election_select_{selected_key}",
            label_visibility="collapsed",
        )

        if a_choice != "선거명을 선택하세요":
            ...
            with ac1:
                raw2 = st.text_input(
                    ...
                    placeholder="입력",
                    key=f"a2_{selected_key}_{a_idx}",
                )
            with ac2:
                raw3 = st.text_input(
                    "결정서 지참자 (3)",
                    ...
                    placeholder="입력",
                    key=f"a3_{selected_key}_{a_idx}",
                )
            with ac3:
                raw4 = st.text_input(
                    "거소투표용지와 회송용봉투 반납자 (4)",
                    ...
                    placeholder="입력",
                    key=f"a4_{selected_key}_{a_idx}",
                )
        ...
        st.markdown(
            ...
            '<span class="small">(투표록2p)</span> &apos;자. 투표용지 수령·교부상황&apos; 기초자료 입력</div>',
            unsafe_allow_html=True,
        )
        st.caption(
            "각 선거별로 남아있는 잔여투표용지 첫번호, 훼손 등으로 미교부한 투표용지 및 그 일련번호를 입력하면, "
            "교부매수 잔여매수 잔여투표용지 일련번호가 자동 계산됩다."
        )
        ...
        if j_choice != "선거명을 선택하세요":
            ...
            with jc1:
                first_raw = st.text_input(
                    ...
                    placeholder="입력",
                    key=f"j_first_{selected_key}_{selected_j_idx}",
                )
            with jc2:
                damaged_raw = st.text_input(
                    ...
                    placeholder="입력",
                    key=f"j_damaged_{selected_key}_{selected_j_idx}",
                )
            with jc3:
                damaged_serial = st.text_input(
                    ...
                    key=f"j_damaged_serial_{selected_key}_{selected_j_idx}",
                )
        ...
        st.info(
            "각 선거의 입력된 내용을 기준으로 투표록 2p 작성 참고자료가 자동 계산됩니다. "
            "자료 입력시 정확하게 입력하시기 바랍니다."
        )
