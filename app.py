...
# BUILD: vote-record-mobile FINAL v3.4 CLICKFIX 2026-08-24
...
st.markdown(
    '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v3.4 · BUILD CLICKFIX</div>',
    unsafe_allow_html=True,
)
st.markdown(
...
def _go_step_v33(step):
    st.session_state.workflow_step_v33 = step


def _menu_item_v33(step, lead, tail, admin=False):
    selected = st.session_state.workflow_step_v33 == step
    with st.container(key=f"nav_{step}_v33"):
        if admin:
            dcol, tcol = st.columns([0.22, 1.78], gap="small")
            with dcol:
                st.button(
                    "●" if selected else "○",
                    key=f"navdot_{step}_v33",
                    on_click=_go_step_v33,
                    args=(step,),
                    help="관리자",
                    width="stretch",
                )
            ...
            dcol, lcol, tcol = st.columns([0.20, 0.62, 1.18], gap="small")
            with dcol:
                st.button(
                    "●" if selected else "○",
                    key=f"navdot_{step}_v33",
...
# 선택된 카드와 선택 동그라미 색상을 현재 상태에 맞춰 직접 지정
_selected = st.session_state.workflow_step_v33
st.markdown(
    f"""
    <style>
    .st-key-nav_{_selected}_v33 {{
        border-color:#ff3434 !important;
        background:#fffafa !important;
    }}
    .st-key-navdot_{_selected}_v33 button p {{
...
with c1:
    _menu_item_v33("select", "①[선택]", " 투표소")
with a1:
    st.markdown('<div class="nav-arrow-v33">➜</div>', unsafe_allow_html=True)
with c2:
    _menu_item_v33("report", "②[보고자료]", " 투표진행상황")
with a2:
    st.markdown('<div class="nav-arrow-v33">➜</div>', unsafe_allow_html=True)
with c3:
    _menu_item_v33("input", "③[입력]", " 투표록 기초자료")
with a3:
    st.markdown('<div class="nav-arrow-v33">➜</div>', unsafe_allow_html=True)
with c4:
    _menu_item_v33("reference", "④[참고]", " 투표록 2p 작성")

admin_col, _ = st.columns([1.15, 5.0])
with admin_col:
    _menu_item_v33("admin", "", "[관리자]", admin=True)
