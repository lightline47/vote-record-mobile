...
# ============================================================
# v3.0 상단/메뉴
# - 메뉴 이동은 Streamlit native button으로 처리하여 session_state 유지
# - 표시 라벨은 별도 HTML로 렌더링하여 부분 색상/굵기 정확히 적용
# - raw <a href=...> 방식 제거: 메뉴 이동 때 로그인 세션 초기화 방지
# ============================================================
st.markdown(
    r"""
    <style>
    /* Streamlit 고정 헤더가 제목을 덮지 않도록 본문을 충분히 내립니다. */
    .block-container {
        max-width: 1380px !important;
    ...
    .workflow-guide-v30 .first-step { color:#ff1717 !important; font-weight:950 !important; }
    .workflow-guide-v30 .next-steps { color:#102ed7 !important; font-weight:950 !important; }
    ...
    .nav-card-label-v30.selected {
        border-color: #ff3434 !important;
        background: #fffafa !important;
    }
    .nav-card-label-v30 .dot {
        font-size: 24px;
        color: #d5d8dc;
        margin-right: 7px;
        flex: 0 0 auto;
    }
    .nav-card-label-v30.selected .dot { color:#ff4b4b !important; }
    .nav-card-label-v30 .purple {
        color:#7b159d !important;
        font-weight:950 !important;
        font-size:18px !important;
    }
    .nav-card-label-v30 .black {
        color:#111 !important;
        font-weight:800 !important;
        font-size:16px !important;
    }
    ...
    .nav-card-label-v30.admin.selected {
        border:2px solid #ff3434 !important;
        padding-left:8px;
        background:#fffafa !important;
    }
    .nav-card-label-v30.admin .black {
        color:#111 !important;
        font-weight:900 !important;
        font-size:18px !important;
    }

    /* 투명한 Streamlit 버튼을 카드 전체 클릭영역으로 겹칩니다.
       버튼이므로 URL 이동이 없고 로그인 session_state가 유지됩니다. */
    .st-key-nav_select, .st-key-nav_report, .st-key-nav_input,
    .st-key-nav_reference, .st-key-nav_admin {
        position: relative !important;
        overflow: visible !important;
    }
    .st-key-nav_select .stButton, .st-key-nav_report .stButton,
    .st-key-nav_input .stButton, .st-key-nav_reference .stButton,
    .st-key-nav_admin .stButton {
        position: absolute !important;
        inset: 0 !important;
    ...
    .st-key-nav_select .stButton > button, .st-key-nav_report .stButton > button,
    .st-key-nav_input .stButton > button, .st-key-nav_reference .stButton > button,
    .st-key-nav_admin .stButton > button {
        width: 100% !important;
    ...
    @media (max-width: 900px) {
        ...
        .app-main-title-v30 { font-size:27px !important; }
        .workflow-guide-v30 { font-size:17px !important; }
        .nav-card-label-v30 { min-height:58px; padding:7px 5px; }
        .nav-card-label-v30 .dot { font-size:19px; margin-right:4px; }
        .nav-card-label-v30 .purple { font-size:14px !important; }
        .nav-card-label-v30 .black { font-size:12px !important; }
        .nav-arrow-v30 { min-height:58px; font-size:25px; }
        .nav-card-label-v30.admin .black { font-size:16px !important; }
    }
    </style>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    '<div class="app-main-title-v30">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v3.0</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="workflow-guide-v30">★ 반드시 '
...
if "workflow_step_v30" not in st.session_state:
    st.session_state.workflow_step_v30 = "select"
if st.session_state.workflow_step_v30 not in WORKFLOW_LABELS:
    st.session_state.workflow_step_v30 = "select"
requested_step = st.session_state.workflow_step_v30
workflow_step = WORKFLOW_LABELS[requested_step]

def _go_step(step):
    st.session_state.workflow_step_v30 = step

def _render_nav_card(step, lead, tail, admin=False):
    selected = st.session_state.workflow_step_v30 == step
    with st.container(key=f"nav_{step}"):
        dot = "●" if selected else "○"
        classes = "nav-card-label-v30"
        if selected:
            classes += " selected"
        if admin:
            classes += " admin"
        st.markdown(
            ...
            f'<span><span class="purple">{lead}</span>'
            f'<span class="black">{tail}</span></span></div>',
            unsafe_allow_html=True,
        )
        if st.button(
            f"{lead}{tail}",
...
# ① → ② → ③ → ④
c1, a1, c2, a2, c3, a3, c4 = st.columns([1.1, .16, 1.45, .16, 1.45, .16, 1.45], gap="small")
with c1:
    _render_nav_card("select", "①[선택]", " 투표소")
with a1:
    st.markdown('<div class="nav-arrow-v30">➜</div>', unsafe_allow_html=True)
with c2:
    _render_nav_card("report", "②[보고자료]", " 투표진행상황")
with a2:
    st.markdown('<div class="nav-arrow-v30">➜</div>', unsafe_allow_html=True)
with c3:
    _render_nav_card("input", "③[입력]", " 투표록 기초자료")
with a3:
    st.markdown('<div class="nav-arrow-v30">➜</div>', unsafe_allow_html=True)
with c4:
    _render_nav_card("reference", "④[참고]", " 투표록 2p 작성")

# 관리자: 별도 줄, 검정색 굵게
admin_col, _ = st.columns([1.1, 4.9])
with admin_col:
    _render_nav_card("admin", "", "[관리자]", admin=True)
