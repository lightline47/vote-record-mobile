...
st.title("🗳️ 투표록 작성 보조 앱 — 모바일 전용 v2.1")
st.caption("투표소를 선택하여 투표록 「아」·「자」 작성용 계산을 할 수 있습니다. 기초자료 관리는 관리자 메뉴에서 할 수 있습니다.")
...
if workflow_step == "① 투표소 선택":
    st.markdown(
        """
        <div class="section-row">
          <span class="section-box">투표소 선택</span>
          <span class="must-select"><span class="must-red">★ 반드시 </span><span class="must-blue">해당동</span><span class="must-red">의 투표소 </span><span class="must-blue">먼저 선택</span><span class="must-red"> ★</span></span>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    if selected_dong and selected_station:
        ...
        if saved_selected_key and saved_selected_key in db:
            ...
            st.markdown(
                ...
                '선택한 투표소는 <span class="selected-name">(　　　　　　　　)</span> 입니다.</div>',
                unsafe_allow_html=True
            )

elif workflow_step == "② 투표진행상황 보고":
    st.markdown(
...
elif workflow_step == "③ 투표록 기초자료 입력":
    if not db:
        ...
        for idx, e in enumerate(station["elections"]):
            ...
            with cols[1]:
                first_raw = st.text_input(
                    "잔여투표용지 첫 번호(NO.)",
                    value=str(saved_j.get(str(idx), {}).get("first_raw", "")),
                    placeholder="입력",
            ...
            with cols[2]:
                damaged_raw = st.text_input(
                    "훼손 등 미교부한 투표용지 매수",
                    value=str(saved_j.get(str(idx), {}).get("damaged_raw", "")),
                    placeholder="입력",
        ...
        for idx, e in enumerate(station["elections"]):
            ...
            with cols[3]:
                raw2 = st.text_input(
                    "거소투표용지 미발송·반송자 (2)",
                    value=str(saved_a.get(str(idx), {}).get("v2_raw", "")),
                    placeholder="입력",
            ...
            with cols[4]:
                raw3 = st.text_input(
                    "결정서 지참자 (3)",
                    value=str(saved_a.get(str(idx), {}).get("v3_raw", "")),
                    placeholder="입력",
            ...
            with cols[5]:
                raw4 = st.text_input(
                    "거소투표용지와 회송용봉투 반납자 (4)",
                    value=str(saved_a.get(str(idx), {}).get("v4_raw", "")),
                    placeholder="입력",
...
elif workflow_step == "④ 투표록 2p 작성(참고)":
    if not db:
        st.warning("기초자료가 없습니다. [관리자] 메뉴에서 엑셀자료를 업로드해 주세요.")
    elif station is None:
        st.warning("먼저 ① 투표소 선택에서 동과 투표소를 선택해 주세요.")
        ...
        for idx, e in enumerate(station["elections"]):
            ...
            if not valid:
                ...
                if damaged_serial:
                    ...
                    serial_text = (
                        f"{remain_range}"
                        f"<br><span style='font-size:13px;'>훼손 등 미교부: {damaged_serial}</span>"
                    )
                else:
