...
# BUILD: vote-record-mobile FINAL v4.6 CONSOLIDATED-UI 2026-08-24
...
st.markdown(
    '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v4.6</div>',
    unsafe_allow_html=True,
)
st.markdown(
...
st.markdown(
    ...
    div[data-testid="stMarkdownContainer"]:has(.workflow-guide-v33) { margin-bottom:-30px !important; }
    div[data-testid="stHorizontalBlock"]:has(.st-key-navcard_select_v35) { margin-top:-18px !important; }
    ...
    /* 입력 기초자료 제목과 안내 크기 */
    .record-input-title .small { font-size:25.3px !important; }
    .election-choice-label span { font-size:21px !important; }
    .record-field-label { color:#1037d7; font-size:16px; font-weight:900; line-height:1.3; margin:0 0 5px 1px; }
    ...
    .record-final-notice .warn { color:#e11; font-size:16.3px; font-weight:950; }
    </style>
    """,
    unsafe_allow_html=True,
)
...
if workflow_step == "①[선택]투표소":
    ...
    if selected_dong and selected_station:
        ...
        changed = local.get("selected_key") != new_selected_key
    ...
    st.markdown(
        ...
        .report-v41-label-red{color:red;font-size:20px;font-weight:900;line-height:1.22;margin-bottom:5px;} .report-v41-election{color:#1768b3;font-size:16px;font-weight:900;margin-left:6px;}
        .report-v41-label-blue{color:blue;font-size:20px;font-weight:900;line-height:1.22;margin-bottom:5px;text-align:center;}
        ...
        @media(max-width:768px){
          ...
          .report-v41-label-red,.report-v41-label-blue{font-size:17px;} .report-v41-label-black{font-size:15px;}
    ...
    if not db or station is None:
        ...
        st.markdown(
            ...
            f'<span class="report-v41-station">({station_display})</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
    ...
    if not db:
        ...
        st.markdown(
            ...
            '자료 입력시 <span class="warn">정확하게 입력하시기 바랍니다.</span></div>',
            unsafe_allow_html=True,
        )
