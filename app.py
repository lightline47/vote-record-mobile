...
# BUILD: vote-record-mobile v5.33 VISUAL-SPACING-CALIBRATION 2026-08-25
...
with title_col_v50:
    st.markdown(
        '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — <span class="mobile-version-v51">모바일 전용 v5.33</span></div>',
        unsafe_allow_html=True,
    )
...
# ============================================================
# v5.32 사용자 지정 줄간격 고정
# 1) 메뉴 네모상자 → 검정 화살표: 2px
# 2) 검정 화살표 → 선택한 투표소 상자: 6px
# 3) 선택한 투표소 상자 → ①[선택] 투표소: 10px
# 4) 안내문 → 보라색 선택상자: 2px
# 손가락 및 기타 정상 영역은 유지
# ============================================================
st.markdown(
...
st.markdown(
    ...
    /* ---------------------------------------------------------
       1. 메뉴 네모상자 → 검정 화살표
       v5.32 실제 화면에서 약 15px로 보였으므로
       화살표 블록 전체를 위로 당겨 약 2px 수준으로 보정
       --------------------------------------------------------- */
    .workflow-progress-arrow-v527 {
    ...
    /* ---------------------------------------------------------
       2. 검정 화살표 → 선택한 투표소 상자
       위 보정 후 실제 시각적 간격이 약 6px이 되도록 조정
       --------------------------------------------------------- */
    div[data-testid="stMarkdownContainer"]:has(.shared-station-confirm) {
        margin-top: 0 !important;
        padding-top: 0 !important;
    ...
    /* ---------------------------------------------------------
       4. 안내문 → 보라색 선택상자
       v5.32 실제 화면에서 약 20px 이상으로 보였으므로,
       보라색 선택상자를 위로 당겨 실제 약 2px로 보정
       --------------------------------------------------------- */
    .st-key-selection_body_v531 .st-key-station_choice_input_box {
        margin-top: -18px !important;
        padding-top: 6px !important;
    ...
    /* 안내문 자체의 불필요한 하단 여백 제거 */
    .select-instruction-v531 {
        margin-bottom: 0 !important;
        padding-bottom: 0 !important;
    }

    /* 선택 본문 Streamlit 자동 gap 제거 */
    .st-key-selection_body_v531 > div[data-testid="stVerticalBlock"] {
        gap: 0 !important;
        row-gap: 0 !important;
    }
