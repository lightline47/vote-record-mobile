...
# BUILD: vote-record-mobile FINAL v4.7 CONSOLIDATED-UI 2026-08-24
...
st.markdown(
    '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — 모바일 전용 v4.7</div>',
    unsafe_allow_html=True,
)
st.markdown(
...
if workflow_step == "①[선택]투표소":
    ...
    if not db:
        ...
        st.markdown(
            '<div class="input-auto-note">거소투표용지 미발송 반송자(2), 결정서 지참자(3), '
            '거소투표용지와 회송용봉투 반납자(4)를 입력하면 투표록 &apos;아.투표상황&apos;의 '
            '선거인명부 등재자(1)와 계(나)가 자동 생성 됩니다.</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="input-auto-note">각 선거별로 남아있는 잔여투표용지 첫번호, 훼손 등으로 미교부한 투표용지 및 그 일련번호를 입력하면, '
            '교부매수 잔여매수 잔여투표용지 일련번호가 자동 계산됩니다.</div>',
            unsafe_allow_html=True,
        )
