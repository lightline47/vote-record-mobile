...
# BUILD: vote-record-mobile v5.76 REPORT-INPUT-NAMEERROR-FIX 2026-08-26
...
with title_col_v50:
    st.markdown(
        '<div class="app-main-title-v33">🗳️ 투표록 작성 보조 앱 — <span class="mobile-version-v51">모바일 전용 v5.56</span></div>',
        unsafe_allow_html=True,
    )
...
if workflow_step == "②[보고]투표진행상황":
        ...
        with st.container(key="report_body_v551"):
            ...
            if not db or station is None:
                ...
                if station["elections"]:
                    ...
                    report_target_name = display_election_name_for_report(report_elections[0]["name"])
                else:
                    report_elections = []
                ...
                with st.container(border=True, key="report_input_calc_box"):
                    ...
                    for i, e in enumerate(report_elections):
                        ...
                        with in1:
                            st.markdown(
                                f'<div class="report-v41-label-red report-input-label-v557">[입력] <span class="report-v41-election">{display_election_name_for_report(e["name"])}</span><br>'
                        ...
                        with in2:
                            st.markdown(
                                f'<div class="report-v41-label-red report-input-label-v557 report-damaged-label-v573">[입력] <span class="report-v41-election">{display_election_name_for_report(e["name"])}</span><br>'
                        ...
                        if raw.strip():
                            try:
                                ...
                                errors.append(f"{display_election_name_for_report(e['name'])}: 잔여투표용지 첫 번호는 숫자만 입력해 주세요.")
                        if damaged_raw.strip():
                            try:
                                ...
                                damaged = 0
                                errors.append(f"{display_election_name_for_report(e['name'])}: 훼손 등 미교부 투표용지 매수는 0 이상의 숫자로 입력해 주세요.")
                        ...
                        if raw.strip():
                            if n is None:
                                ...
                                invalid_input = True
                                errors.append(f"{display_election_name_for_report(e['name'])}: 허용범위 {e['start_no']:,} ~ {e['end_no']+1:,}")
                            else:
                                ...
                                if cumulative < 0:
                                    invalid_input = True
                                    errors.append(
                                        f"{display_election_name_for_report(e['name'])}: 훼손 등 미교부 매수가 현재까지 진행된 일련번호 수보다 많습니다."
                                    )
                                    cumulative = 0
                                else:
                ...
                for e in station["elections"]:
                    actual_election_name = str(e.get("name", "")).strip()
                    ref_rows.append(
                        "<tr>"
    ...
    with workflow_cluster_v59:
        if not db:
            ...
            st.markdown("""<div class="reference-validation-title-v569"><span class="validation-star-v569">★검증★</span> [아. '계(나)' = 자. '교부매수(라)']</div>""", unsafe_allow_html=True)
            all_match = True
            any_ready = False
    ...
    if not st.session_state.admin_authenticated:
        ...
        st.caption("②[보고] 투표진행상황 화면의 '지역구국회의원선거' 자리에 표시할 선거명을 입력합니다.")
        current_report_name = load_report_election_name()
        report_name_input = st.text_input(
            "보고할 선거명",
            value=current_report_name,
            key="admin_report_election_name_v571",
            placeholder="예: 북구을국회의원선거",
        )
        if st.button("💾 보고할 선거명 저장", width="stretch", key="save_report_election_name_v571"):
            cleaned_report_name = str(report_name_input or "").strip()
            if not cleaned_report_name:
        ...
        if not st.session_state.user_input_reset_confirm_mode:
            if st.button("♻️ 앱 사용자 입력내용 리셋", width="stretch", key="user_input_reset_start_v582"):
            ...
            reset_pw = st.text_input(
                "리셋 확인 관리자 비밀번호",
                type="password",
                key="user_input_reset_password_v582"
            )
            rc1, rc2 = st.columns(2)
            with rc1:
                if st.button("리셋 실행", width="stretch", key="user_input_reset_execute_v582"):
                    if reset_pw == current_admin_password():
                        # 엑셀 기초자료/설정은 유지하고 앱 사용자가 입력한 데이터만 삭제합니다.
                        local["hourly_by_station"] = {}
                        local["record_inputs_by_station"] = {}
                        save_local()

                        # 현재 브라우저 세션에 남아 있는 입력 위젯/계산 결과도 함께 비웁니다.
                        reset_prefixes = (
                            "cur_text_",
                            "report_damaged_",
                            "a2_", "a3_", "a4_",
            ...
            with rc2:
                if st.button("리셋 취소", width="stretch", key="user_input_reset_cancel_v582"):
                    st.session_state.user_input_reset_confirm_mode = False
                    st.rerun()
